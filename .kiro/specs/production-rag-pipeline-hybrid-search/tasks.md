...
- [x] 2. Document normalizers
  ...
  - [x]* 2.6 Write property test: Normalization preserves document metadata
  ...
  - [x]* 2.7 Write unit tests for each normalizer format
...
- [x] 3. Chunking strategies
  ...
  - [x]* 3.5 Write property test: Fixed-size chunker size and overlap invariants
  ...
  - [x]* 3.6 Write property test: Recursive chunker respects section boundaries
  ...
  - [x]* 3.7 Write property test: Semantic chunker splits at similarity threshold
  ...
  - [x]* 3.8 Write property test: All chunks carry required metadata
...
- [x] 4. Checkpoint — Core processing layer
  - Ensure all tests pass, ask the user if questions arise.

- [x] 5. Infrastructure adapters — storage layer
  - [x] 5.1 Implement `LocalDocumentStore` adapter (DocumentStorePort)
    - Store/retrieve raw documents on local filesystem for dev; S3 adapter as alternate
    - _Requirements: 1.3_

  - [x]* 5.2 Write property test: Raw document storage round-trip
  ...
  - [x] 5.6 Implement `OpenAIEmbedding` adapter (EmbeddingPort)
    - embed() and embed_single() using text-embedding-3-small
    - Batch embedding with rate limit handling
    - _Requirements: 3.1_

  - [x] 5.7 Implement `OpenAILLM` adapter (LLMPort) and `AnthropicLLM` fallback adapter
    - generate() and generate_structured() for GPT-4o primary
    - Fallback adapter for Claude Sonnet
    - _Requirements: 5.1_

  - [x] 5.8 Implement `CrossEncoderReranker` adapter (RerankerPort)
  ...
  - [x] 5.9 Implement `RedisCache` adapter (CachePort)
    - get(), set() with TTL, invalidate() by pattern
    - _Requirements: 15.11_

- [x] 6. Indexing service with deduplication
  ...
  - [x] 6.2 Implement deduplication logic
  ...
  - [x] 6.3 Implement re-indexing flow
    - Remove all prior chunks from all indexes for a document, then re-index with new strategy
    - _Requirements: 3.6_

  - [x]* 6.4 Write property test: Deduplication rejects near-duplicate chunks
  ...
  - [x]* 6.5 Write property test: Re-chunking produces valid output from stored content
...
- [x] 7. Entity extraction via Instructor
  - [x] 7.1 Implement `EntityExtractor` with Instructor + Pydantic schemas
  ...
  - [x]* 7.2 Write unit tests for entity extraction
...
- [x] 8. Ingestion service (orchestrator)
  - [x] 8.1 Implement `IngestionService`
  ...
  - [x]* 8.2 Write property test: Successful ingestion produces correct domain event
  ...
  - [x]* 8.3 Write unit tests for ingestion error paths
    - Unsupported format rejection, corrupted file handling, path traversal detection
    - _Requirements: 1.4, 1.5, 1.7_

- [x] 9. Checkpoint — Ingestion pipeline complete
  - Ensure all tests pass, ask the user if questions arise.

- [x] 10. Retrieval service with RRF fusion and reranking
  - [x] 10.1 Implement `RetrievalService` with three-way parallel retrieval
  ...
  - [x] 10.2 Implement `reciprocal_rank_fusion()` function
  ...
  - [x] 10.3 Implement reranking step
  ...
  - [x]* 10.4 Write property test: RRF fusion with configurable weights
  ...
  - [x]* 10.5 Write property test: Reranker selection maintains ordering
  ...
  - [x]* 10.6 Write property test: Retrieval results include required metadata
...
- [x] 11. Generation service with citation verification
  - [x] 11.1 Implement `GenerationService`
  ...
  - [x] 11.2 Implement `CitationVerifier` (LLM-as-judge)
  ...
  - [x]* 11.3 Write property test: Citation format correctness
  ...
  - [x]* 11.4 Write property test: Citation verification and scoring
  ...
  - [x]* 11.5 Write property test: Token budget enforcement
...
- [x] 12. Confidence scoring and fallback
  - [x] 12.1 Implement `ConfidenceService`
  ...
  - [x] 12.2 Implement fallback response generation
  ...
  - [x]* 12.3 Write property test: Confidence score structure and validity
  ...
  - [x]* 12.4 Write property test: Low confidence triggers fallback
...
- [x] 13. Checkpoint — Core pipeline services complete
  - Ensure all tests pass, ask the user if questions arise.

- [x] 14. Security service
  - [x] 14.1 Implement `SecurityService` — prompt injection detection
  ...
  - [x] 14.2 Implement document content scanning
  ...
  - [x] 14.3 Implement input validation (filename, path traversal, injection)
  ...
  - [x] 14.4 Implement PII detection and handling policy
    - Detect PII in ingested documents, apply configured policy (flag/quarantine)
    - _Requirements: 1.8_

  - [x] 14.5 Implement token budget management
  ...
  - [x] 14.6 Implement output sanitization
    - Sanitize generated output to prevent insecure rendering (strip HTML/JS injection)
    - _Requirements: 11.4_

  - [x]* 14.7 Write property test: Prompt injection detection
...
- [x] 15. API layer (FastAPI routes, middleware, error handling)
  - [x] 15.1 Create OpenAPI 3.1 specification (openapi.yaml)
  ...
  - [x] 15.2 Implement FastAPI application scaffold and dependency injection
    - App factory, lifespan events, DI container for services
    - CORS, trusted hosts configuration
    - _Requirements: 8.1_

  - [x] 15.3 Implement `POST /v1/ask` endpoint
  ...
  - [x] 15.4 Implement `POST /v1/ingest` endpoint
  ...
  - [x] 15.8 Implement global error handler middleware
  ...
  - [x] 15.9 Implement request validation middleware
    - Validate all inputs against OpenAPI schema, return 422 for invalid requests
    - _Requirements: 8.7_

  - [x] 15.10 Implement rate limiting middleware
    - Configurable limits per role and per user
    - Return 429 Too Many Requests with Retry-After header
    - _Requirements: 8.9, 15.7_

  - [x]* 15.11 Write property test: API input validation rejects invalid requests
  ...
  - [x]* 15.12 Write property test: Error responses contain required structure
...
- [x] 17. Authentication and RBAC
  - [x] 17.1 Implement OAuth2/OIDC authentication middleware
    - Validate JWT tokens, extract user identity and roles
    - Exempt /health and /ready from auth requirements
    - _Requirements: 10.1_

  - [x] 17.2 Implement RBAC enforcement
...
- [x] 18. Resilience layer (circuit breakers, retries, degradation)
  ...
  - [x] 18.2 Implement retry with exponential backoff and jitter
    - `RetryConfig` with max_retries, base_delay, max_delay, jitter
    - Apply to all retriable external calls
    - _Requirements: 15.3_

  - [x] 18.3 Wire circuit breakers into adapters
  ...
  - [x]* 18.4 Write integration tests for degraded mode behavior
...
- [x] 19. Observability
  ...
  - [x] 19.4 Implement token usage and cost tracking
  ...
  - [x] 19.5 Create Grafana dashboard definitions (JSON/YAML)
    - System health, RAG-specific metrics, cost tracking panels
    - _Requirements: 14.4_

  - [x] 19.6 Define SLOs, SLIs, and error budgets
  ...
  - [x] 19.7 Implement telemetry export to Parquet/Iceberg
    - Export observability data, cost logs, and metrics on configurable schedule
    - _Requirements: 14.9_

- [x] 20. Checkpoint — Backend services fully integrated
  - Ensure all tests pass, ask the user if questions arise.

- [x] 21. Evaluation framework
  - [x] 21.1 Create golden dataset structure and initial Q&A pairs
  ...
  - [x] 21.2 Implement evaluation harness
  ...
  - [x] 21.3 Implement regression detection and CI integration
...
- [x] 22. Web UI — React + TypeScript dashboard
  ...
  - [x] 22.3 Implement search results panel
  ...
  - [x] 22.4 Implement confidence score display
  ...
  - [x] 22.5 Implement accessibility compliance
  ...
  - [x] 22.6 Implement error states and mobile responsiveness
  ...
  - [x]* 22.7 Write unit tests for React components
...
- [x] 24. Infrastructure as Code (Terraform)
  - [x] 24.1 Set up Terraform project structure
...
- [x] 26. CI/CD pipeline (GitLab CI)
  ...
  - [x] 26.3 Implement contract testing stage
    - Validate API implementation against OpenAPI spec (Schemathesis)
    - Fail on divergence
    - _Requirements: 8.6_

  - [x] 26.4 Implement DAST and API security testing stage
  ...
  - [x] 26.5 Implement evaluation and performance stages
  ...
  - [x] 26.6 Implement deployment and rollback
  ...
  - [x] 26.7 Implement branch protection and notifications
  ...
  - [x] 26.8 Implement E2E UI tests stage (Playwright)
    - Desktop and mobile viewport tests
    - _Requirements: 16.9_

- [x] 27. GRC artifacts and compliance mapping
  - [x] 27.1 Create ISO 27001 control matrix
  ...
  - [x] 27.2 Create ISO 42001 AI control matrix
